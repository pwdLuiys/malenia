...
def write_registry_state(tweak, turn_on=True):
    ...
    try:
        with winreg.CreateKeyEx(
            tweak["hive"], tweak["key"], 0, winreg.KEY_SET_VALUE
        ) as key:
...
def action_visual_profiles():
    ...
    print(
        "2. Smooth / Performance (Disables animations and shadows for max performance)"
    )
...
def action_ultimate_performance():
    ...
    try:
        ...
        result = subprocess.run(
            cmd_duplicate, capture_output=True, text=True, check=True
        )

        # Extracts the dynamically generated GUID using regex to activate it immediately.
        match = re.search(
            r"([0-9a-fA-F]{8}(-[0-9a-fA-F]{4}){3}-[0-9a-fA-F]{12})", result.stdout
        )
        ...
        if match:
            ...
            print(
                "[Success] Ultimate Performance plan has been successfully activated."
            )
        else:
...
def action_edge_debloat():
    ...
    desktop_path = os.path.join(
        os.environ["USERPROFILE"], "Desktop", "Microsoft Edge.lnk"
    )
...
def action_destroy_windows_ai():
    ...
    ai_registry_policies = [
        ...
        (
            winreg.HKEY_CURRENT_USER,
            r"Software\Microsoft\Windows\CurrentVersion\Applets\Paint",
            "FeatureOverrides",
            0,
        ),
        ...
        (
            winreg.HKEY_LOCAL_MACHINE,
            r"SOFTWARE\Policies\Microsoft\Windows\SnippingTool",
            "DisableClickToDo",
            1,
        ),
        ...
        (
            winreg.HKEY_CURRENT_USER,
            r"Software\Policies\Microsoft\Office\16.0\Common\ExperimentConfigs\ExternalFeatureOverrides\copilot",
            "State",
            0,
        ),
    ]
...
def action_remove_onedrive():
    ...
    if os.path.exists(executable):
        ...
        print(
            "[Warning] The OneDrive uninstaller executable was not found (likely already removed)."
        )
...
def prompt_tweaks_menu():
    ...
    user_input = input(
        "\nEnter the numbers of the tweaks to be modified separated by space (e.g., '1 4 9'): "
    ).strip()
    if not user_input:
